...
namespace Cariacity.game
{
    ...
    public class Building
    {
        ...
        protected static bool IsBuildable(GridCell cell, Rectangle bounds)
        {
            ...
            {
                try { _obj = Common.Matrix[i, y0 - 1].obj; } 
                catch (IndexOutOfRangeException) { _obj = null; }

                if (_obj != null && _obj.tag == Tags.Street)
                    return true;

                try { _obj = Common.Matrix[i, y1 + 1].obj; } 
                catch (IndexOutOfRangeException) { _obj = null; }
            ...
            {
                try { _obj = Common.Matrix[x0 - 1, j].obj; } 
                catch (IndexOutOfRangeException) { _obj = null; }

                if (_obj != null && _obj.tag == Tags.Street)
                    return true;

                try { _obj = Common.Matrix[x1 + 1, j].obj; } 
                catch (IndexOutOfRangeException) { _obj = null; }
